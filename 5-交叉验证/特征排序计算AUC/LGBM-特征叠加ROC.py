...
# 加载CSV文件
data = pd.read_csv('E:/RS/2分类/特征筛选/LGBM/LGBM-SHAP-特征.csv')
...
for num_features in range(1, 24):  # 从1个特征到23个特征
    ...
    for train_index, test_index in cv.split(X_selected, y):
        ...
        # 初始化LightGBM模型
        model = LGBMClassifier(n_estimators=100, random_state=42)  # 使用100棵树
...
roc_df.to_csv('E:/RS/2分类/特征筛选/LGBM/feature_roc_results_cv.csv', index=False)
